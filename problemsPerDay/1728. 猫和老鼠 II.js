...
class canMouseWin {
    constructor(grid, catJump, mouseJump) {
        ...
        const degrees = new Array(total)
            .fill(0)
            .map(() =>
                new Array(total).fill(0).map(() => new Array(2).fill(0))
            );
        const results = new Array(total)
            ...
            .map(() =>
                new Array(total)
                    .fill(0)
                    .map(() =>
                        new Array(2).fill(0).map(() => new Array(2).fill(0))
                    )
            );
        const queue = [];
        // 计算每个状态的度
                ...
                if (results[prevMouse][prevCat][prevTurn][0] === UNKNOWN) {
                    ...
                    if (canWin) {
                        ...
                        if (degrees[prevMouse][prevCat][prevTurn] === 0) {
                            ...
                            results[prevMouse][prevCat][prevTurn][0] =
                                loseResult;
